...
**CAFE Format Version:** 1.0 (frozen 2026-09-04 — independent of the implementation's own version, see `docs/CAFE-spec.md` section 13 and the compatibility table below)
**Specification:** `docs/CAFE-spec.md`
**Implementation:** `cafe-rs` v1.12.0, Rust 2021, BSD-3-Clause license

> **Two version numbers, on purpose.** The *CAFE Format* (the on-disk byte layout) and `cafe-rs` (this Rust crate/CLI) are versioned independently. The format only bumps for a breaking change or a normative extension; the crate bumps on every user-visible release regardless of whether the on-disk format changed at all. See `docs/CAFE-spec.md` section 13 for the full rationale, and the table below for which `cafe-rs` release introduced which format-relevant feature.

### Implementation ↔ Format compatibility table

| `cafe-rs` version | CAFE Format | Notes |
|---|---|---|
| v1.0 | 1.0 | Original format baseline: `IHDR`/`PLTE`/`IDAT`/`IEND`, ZSTD, filters 0–13, `iDIM`, Adam7 + even/odd interlace, all v1.0 ancillary chunks, uint/float/half sample formats. |
| v1.1 | 1.0 | Filters 14–15, byte-shuffle (`Filter method = 1`), real 2D-tiling round-trip, HDR tone-mapping on decode, AVX2 SIMD (filters 1–3) — filters/byte-shuffle are on-disk additions, retroactively folded into Format 1.0 at the v1.12.0 freeze (section 13.2); SIMD itself has zero on-disk effect. |
| v1.2 – v1.4.2 | 1.0 | Pure implementation work: broader SIMD coverage (AVX2 + ARM NEON), CI cross-compile/native-ARM jobs. No on-disk change. |
| v1.5 | 1.0 | Per-row predictive filter (`Filter method = 3`) is an on-disk addition, folded into Format 1.0; compression benchmarks, `auto_dictionary` guarantee, redmean palette, and the `tile_rows` tuning investigation are pure implementation work. |
| v1.6 – v1.6.3 | 1.0 | Streaming `Encoder<W>`/`Decoder<R>` APIs, CLI flags, real `compression_stats`, nightly fuzz CI — all produce/consume bytes byte-for-byte identical to the whole-file `encode()`/`decode()` path. No on-disk change. |
| v1.7 – v1.9.3 | 1.0 | k-means palette quantization, inverse tone-mapping on encode, streaming-decode 2D tiling, ARM64 native CI, `compression_method` semantics documentation — all pure implementation/documentation work. No on-disk change. |
| v1.10 – v1.11 | 1.0 | Streaming-encode 2D tiling (`add_idim_tile()`) and even/odd interlacing (`add_even_odd_rows()`) — both verified byte-for-byte identical to the whole-file `encode()` output for equivalent inputs. No on-disk change. |
| v1.12 | 1.0 | Format-versioning documentation/tooling: `FORMAT_VERSION_MAJOR`/`FORMAT_VERSION_MINOR` constants, `-V`/`--version` CLI flag, this table and `docs/CAFE-spec.md` section 13 itself — the CAFE Format 1.0 freeze described throughout this table is declared *by* this release, not a new on-disk feature *of* it. No on-disk change. |

**Reading this table**: every `cafe-rs` release to date implements **Format 1.0** — no release has ever required a decoder to understand a format version newer than 1.0 to read files any other release produced. A future format-breaking or format-extending change would introduce a new row with a bumped Format column (e.g. `1.1` for a compatible extension, `2.0` for a break) independent of whatever `cafe-rs` version number happens to ship it.
...
| `tonemap_operator` | — | ⚠️ **partial** | This field only affects *decode* (`decode_bytes_with_opts`/`decode_with_opts`, wired via `cafe-decode`'s `--tonemap-operator`); there is no *encode*-side flag for it, nor should there be — it is not consulted by `encode()` at all. See `inverse_tonemap` below for the actual encode-side tone-mapping option (a different field entirely). |
| `inverse_tonemap` | `--inverse-tonemap <reinhard>` | ✅ | v1.8, opt-in inverse tone-mapping (ITM): synthesizes HDR float data from SDR input instead of the naive `v/255`. Only `reinhard` supported (no closed-form inverse for `filmic`); requires `--sample-format 1` + `--chdr-transfer 0` + RGBA color type. |
...
| **v1.8** | **Inverse tone-mapping on encode (SDR→HDR synthesis)**: `ToneMapOperator::apply_inverse` (Reinhard only — no closed-form inverse for Filmic/ACES), `tonemap::apply_inverse_tone_mapping_to_image`, opt-in `EncodeOptions::inverse_tonemap: Option<ToneMapOperator>` field (default `None`, non-breaking), `--inverse-tonemap reinhard` CLI flag on `cafe-encode`; requires `sample_format=1` (float) + `chdr_transfer=0` (linear only) + RGBA color type — see "v1.8" notes below | ✅ |
| **v1.9** | **`Decoder<R>::next_tile()` support for 2D tiling (`iDIM`)**: `next_tile()` now yields one `Tile` per `IDAT` with its real `(x, y, width, height)` grid position (row-major or Z-order, partial edge tiles included) instead of unconditionally erroring for `iDIM` files; `decode_idim_tile_raw` shared between the whole-image and streaming paths so tile geometry/filter-reversal logic can't diverge; interlace (Adam7/even-odd) support remains a permanent, documented design limitation (not a gap) — see "v1.9" notes below | ✅ |
| **v1.9.1** | **Documentation-only: `Encoder<W>`'s missing `auto_dictionary`/indexed/interlace support reclassified from "v1 gap" to permanent, investigated design limitation** — no code or behavior change; `EncoderOptions`'s and `Encoder<W>`'s doc comments rewritten with per-item technical reasoning for why no incremental, buffer-free implementation exists for any of the three — see "v1.9.1" notes below | ✅ |
| **v1.9.2** | **CI: ARM64 native test job** — new `arm64-native-test` job in `.github/workflows/ci.yml` runs `cargo test --lib --release` + full integration suite on `ubuntu-24.04-arm` (a GitHub-hosted runner backed by real Arm server CPUs, not x86_64-with-emulation), closing the gap between `aarch64-cross-compile`'s type-check-only coverage and v1.4.1's one-off manual QEMU validation — NEON intrinsics now actually execute, on real ARM64 silicon, on every push/PR — see "v1.9.2" notes below | ✅ |
| **v1.9.3** | **Documentation-only: `compression_method` (`IHDR`) spec semantics clarified** — `bit0` normatively defined as a required *lower bound* on codecs a decoder must support (never a per-chunk record — that's `Flag`'s job), with a new decoder-conformance note on the interoperability hazard for independent decoders that skip initializing ZSTD based on `bit0` alone; no code or runtime behavior change — see "v1.9.3" notes below | ✅ |
| **v1.10** | **`Encoder<W>::add_idim_tile()` — 2D tiling (`iDIM`) support for the streaming encoder**: symmetric encode-side counterpart to v1.9's streaming-decode `iDIM` support; corrects v1.9.1's classification of `iDIM` as a permanent `Encoder<W>` limitation (unlike `auto_dictionary`/indexed/interlace, `iDim::tile_order()` needs only geometry already known to `Encoder::new()`, not pixel data) — new `EncoderOptions::idim: Option<(u16, u16, u8)>` field (default `None`, non-breaking), row-major and Z-order both supported, `add_tile()`/`add_idim_tile()` mutually exclusive and cross-calling either is a caller error — see "v1.10" notes below | ✅ |
| **v1.11** | **`Encoder<W>::add_even_odd_rows()` — even/odd interlacing (`Interlace = 2`) support for the streaming encoder**: revisits v1.9.1's bundling of Adam7 and even/odd together as a permanent `Encoder<W>` limitation — even/odd's two passes are each independently, losslessly reconstructable from any row subset (unlike Adam7, whose passes scatter pixels non-contiguously across the whole image and genuinely cannot be streamed); new `EncoderOptions::even_odd_interlace: bool` field (default `false`, non-breaking, mutually exclusive with `idim`/`use_filter_per_row`/`use_byte_shuffle`), restricted to 8-bit uint RGBA (same restriction interlace already has on the whole-file `encode()` path); `add_even_odd_rows()` accepts arbitrary-height, arbitrary-alignment contiguous row ranges and may split a single pass across multiple `IDAT`s as rows arrive — decoder-side concatenation-by-`pass_number` documented in the spec; Adam7 remains permanently unsupported by `Encoder<W>` — see "v1.11" notes below | ✅ |
| Future | Real hardware validation on physical *end-user* ARM devices (Raspberry Pi, mobile, Apple Silicon — distinct from v1.9.2's cloud-hosted-but-real-silicon CI coverage), additional compressors, tone-mapping operator selection via CLI for PQ/HLG/sRGB transfer functions and Filmic on encode | ⏳ |
...
3. **Advanced 2D tiling** — iDIM with per-tile IDAT already implemented (row-major and Z-order), including streaming decode via `Decoder<R>::next_tile()` (v1.9) and streaming encode via `Encoder<W>::add_idim_tile()` (v1.10); evolve with preview/progressive streaming, and richer per-tile progress/callback hooks
...
**Last updated:** September 4, 2026 | **Project version:** v1.12.0 | **ARM NEON SIMD Phase (Sep 1/2026) + Compression-Focused Audit (Sep 2/2026) + Streaming Encoder (Sep 3/2026) + CLI ICC/XMP flags (Sep 3/2026, v1.6.1) + Real compression_stats + cafe-decode export flags (Sep 3/2026, v1.6.2) + Nightly fuzz CI workflow (Sep 3/2026, v1.6.3) + k-means palette quantization (Sep 3/2026, v1.7) + Inverse tone-mapping on encode (Sep 3/2026, v1.8) + Streaming decode of 2D tiling (Sep 4/2026, v1.9) + `Encoder<W>` limitations reclassified as permanent (Sep 4/2026, v1.9.1) + CI: ARM64 native test job (Sep 4/2026, v1.9.2) + `compression_method` spec semantics clarified (Sep 4/2026, v1.9.3) + Streaming encoder 2D tiling support (Sep 4/2026, v1.10) + Streaming encoder even/odd interlacing support (Sep 4/2026, v1.11) + CAFE Format Version formalized, decoupled from `cafe-rs` version (Sep 4/2026, v1.12):**

### v1.12 - CAFE Format Version formalized, decoupled from `cafe-rs` implementation version (documentation + tooling only)

The project's release cadence (v1.6 through v1.11 shipped within a matter of days) made it easy to mistake `cafe-rs`'s own SemVer for a signal about on-disk compatibility, when in nearly every one of those releases the on-disk bytes never changed at all — a distinction that was already true in practice (see the "Implementation ↔ Format compatibility table" above) but had never been formalized into an explicit, documented rule, nor exposed anywhere a user or tool could check it without reading source code.

- **New public constants `FORMAT_VERSION_MAJOR`/`FORMAT_VERSION_MINOR`** (`src/constants.rs`, re-exported from `src/cafe.rs`): `1`/`0`. Deliberately **not** a byte written anywhere in a `.cafe` file — section 13.3 of `docs/CAFE-spec.md` documents the rationale in full (mirrors PNG's own decades-long precedent of no `IHDR` version field: the chunk/enum/ancillary-metadata extensibility mechanisms the format already has cover forward/backward compatibility without one, and a version field would face the same chicken-and-egg problem PNG's own spec discusses — an old decoder can't be relied on to check a new version field it doesn't know to look for). These two constants exist purely so the CLI and any future programmatic capability check have one source of truth instead of a version string duplicated independently (and inevitably drifting out of sync, as happened here) across the CLI, the spec, and this file.
- **New `-V`/`--version` flag on both `cafe-encode` and `cafe-decode`** (`tools/cafe-encode.rs`/`tools/cafe-decode.rs`): prints the crate's own SemVer (`CARGO_PKG_VERSION`, e.g. `cafe-encode 1.12.0`) on one line and `CAFE format {FORMAT_VERSION_MAJOR}.{FORMAT_VERSION_MINOR}` (e.g. `CAFE format 1.0`) on the next — two numbers, deliberately kept visually distinct, so a user diagnosing "can this build read/write files another CAFE tool produced" looks at the right one instead of the crate's own release number.
- **New `docs/CAFE-spec.md`/`.pt.md` section 13 ("Versioning")**: normatively defines the CAFE Format Version (`MAJOR.MINOR`, no patch component — a wire format has no "bugfix releases" the way a crate does) as tracking only breaking changes or normative extensions, independent of `cafe-rs`'s own per-release SemVer bump. Declares **CAFE Format 1.0 frozen as of this release (2026-09-04)**, retroactively encompassing every on-disk addition shipped between the original v1.0 and v1.11 (filters 14–15, byte-shuffle, per-row filtering, 2D tiling, even/odd interlacing — all real wire-format additions, but never previously released under a distinct format-version label of their own). Section 13.2 walks through the full v1.0–v1.11 history classifying each release's changes into "genuine on-disk addition" vs. "pure implementation work" (SIMD, streaming APIs, CLI flags, CI, palette algorithms, tone-mapping, documentation); section 13.3 covers why `IHDR` still has no version field and what a hypothetical future Format 2.0 would look like instead (a new file signature, not a repurposed/extended `IHDR` byte).
- **New `AGENTS.md` "Implementation ↔ Format compatibility table"** (this file, above): the scannable, at-a-glance counterpart to the spec's section 13.2 prose — maps every `cafe-rs` release from v1.0 through v1.12 to the CAFE Format version it implements (all of them: 1.0 — this release included, since it adds no on-disk feature of its own), for future contributors deciding whether their own change needs a format-version bump.
- **`README.md`/`README.pt.md`, `docs/DEVELOPER_GUIDE.md`**: version headers reworded to show the implementation version and the CAFE Format version as two separate, clearly-labeled lines instead of a single ambiguous "Version"/"Versão" field. This pass also caught and fixed both READMEs' "Version"/"Versão" header having read stale `1.7.0`-era values (and a stale "30 tests" streaming-encoder count from v1.10) despite the crate having advanced to v1.11.0 across four releases in between — a concrete, small-scale illustration of exactly the kind of silent version drift this whole change is meant to prevent going forward.
- **No on-disk/behavioral change whatsoever**: `FORMAT_VERSION_MAJOR`/`_MINOR` are read-only constants and `-V`/`--version` is a new, additive CLI flag; every existing `EncodeOptions`/`EncoderOptions`/`DecodeResult` field, chunk layout, and byte-for-byte encode/decode behavior is untouched. Versioned as a `cafe-rs` **minor** bump (new public API surface: the two constants plus the CLI flag) rather than a patch, per the same SemVer discipline this release's own section 13 asks future contributors to apply to the *format* number.

**Validation:** `cargo build --lib`, `cargo build --bins`, `cargo fmt --check`, `cargo clippy --all-targets -- -D warnings`, `cargo test --lib` (332 tests, unchanged), full workspace `cargo test` (unchanged pass count across all suites) all pass with zero regressions. Manually verified `cafe-encode --version`/`cafe-decode --version` both print `cafe-encode 1.12.0`/`cafe-decode 1.12.0` followed by `CAFE format 1.0`.

### v1.10 - `Encoder<W>::add_idim_tile()` — 2D tiling (`iDIM`) support for the streaming encoder

Closes the "Advanced 2D tiling" item from "Welcome Contributions" #3's remaining `Encoder<W>` write-side gap, and corrects v1.9.1's classification of `iDIM` as a permanent `Encoder<W>` limitation: `Decoder<R>::next_tile()` gained streaming `iDIM` support in v1.9, but the encode-side counterpart didn't exist yet. Investigating it (rather than assuming it belonged in the same "permanent limitation" bucket as `auto_dictionary`/indexed-palette/interlace) revealed it doesn't need whole-image pixel data upfront — only `tile_width`/`tile_height`/`scan_order`, which combined with the `width`/`height` already required by `Encoder::new()` is enough to compute the full tile grid and write the `iDIM` chunk before a single pixel arrives.

- **`EncoderOptions::idim: Option<(u16, u16, u8)>`** (new field, `src/types.rs`; default `None`, non-breaking via the existing `..Default::default()` pattern used at every construction site): `Some((tile_width, tile_height, scan_order))` switches an `Encoder` from row-strip mode to 2D-tiling mode. A tuple (not `iDim` directly) since `tiles_x`/`tiles_y` are derived from `width`/`height`, not caller-supplied.
- **`Encoder::new()`** (`src/cafe.rs`) validates `opts.idim` upfront, before writing the signature or any chunk: `bit_depth >= 8` (iDIM's per-tile filtering/byte-shuffle logic assumes byte-aligned samples, same restriction `encode()`'s whole-image iDIM path and the decoder's `handle_idim_chunk` already enforce), incompatible with `use_filter_per_row` (mirrors `encode()`'s own check), `tile_width`/`tile_height` nonzero, `scan_order <= 1`, and `tiles_x as u64 * tiles_y as u64 <= MAX_TILE_COUNT` (the same CWE-789 ceiling `handle_idim_chunk` enforces on decode, checked here on the encode side for symmetry — an encoder could otherwise be asked to allocate a `tile_order()` vector as large as a malicious `iDIM` chunk could produce on decode). On success, writes the `iDIM` chunk immediately after `IHDR` (section 9's mandatory chunk order — same placement as `append_idim_chunk_if_present`'s whole-image equivalent) and precomputes `idim.tile_order()`'s sequence into a new `idim_tile_order: Vec<(u16, u16)>` field, tracked alongside a new `idim_next_tile_idx: usize` cursor.
- **`Encoder::add_idim_tile(&mut self, rgba_tile: &[u8]) -> Result<()>`** (new method): encodes exactly one full rectangular tile per call, in `idim_tile_order`'s sequence — looks up the next `(tx, ty)` position, computes its real `(tile_w, tile_h)` via `iDim::tile_dimensions()` (narrower/shorter at the last row/column when `width`/`height` aren't exact multiples of the declared tile size, exactly mirroring `decode_idim_tile_raw`'s decode-side edge-tile handling), validates `rgba_tile.len()` matches that tile's expected size exactly, then runs the same color-conversion → `apply_single_tile_filter` → `compress_with_fallback_dict` → write-`IDAT` pipeline `add_tile()` already uses (both share `apply_single_tile_filter`, so byte-shuffle/predictive-filter behavior is identical between row-strip and iDIM modes). Returns `UnsupportedFeature` if called on a row-strip-mode `Encoder`, if every tile in the grid has already been submitted, or on a buffer-size mismatch.
- **`add_tile()`** gained a guard returning `UnsupportedFeature` when called on an iDIM-mode `Encoder` (pointing the caller at `add_idim_tile()`) — the two submission modes are mutually exclusive; calling the wrong one is a caller error, not silently ignored or auto-corrected.
- **New private `is_complete(&self) -> bool` helper**: `idim_next_tile_idx == idim_tile_order.len()` in iDIM mode, `rows_written == height` in row-strip mode (unchanged) — both `finish()` and `finish_exact()` now call this instead of directly comparing `rows_written`, with mode-specific error messages naming the correct submission method (`add_tile()` vs `add_idim_tile()`) when incomplete.
- **`EncoderOptions`'s doc comment** (`src/types.rs`) gained a new paragraph explaining why `iDIM` turned out *not* to require whole-image buffering (unlike the three genuinely permanent limitations, whose per-item reasoning from v1.9.1 is otherwise unchanged); **`Encoder<W>`'s own doc comment** (`src/cafe.rs`) gained a "# 2D tiling (`iDIM`, v1.10+)" section describing the `new()`/`add_idim_tile()` contract and updated its "Permanent limitations" list to no longer include `iDIM`.
- **`examples/streaming_encode.rs`** extended with an optional 4th CLI argument (`<tile_w>x<tile_h>` or `<tile_w>x<tile_h>:z` for Z-order) demonstrating `add_idim_tile()` end-to-end, alongside the pre-existing row-strip default; its stale "Limitations" doc comment (which previously said "no `iDIM` 2D tiling") was corrected.
- **New tests** (`tests/streaming_encode.rs`, +12): `test_streaming_encoder_idim_row_major_roundtrip` / `test_streaming_encoder_idim_zorder_roundtrip` (pixel-exact round-trip on a non-tile-aligned 33×23 image with 8×8 tiles, exercising partial edge tiles, via `finish()`/`finish_exact()` respectively), `test_streaming_encoder_idim_matches_whole_file_encode_byte_for_byte` (confirms `add_idim_tile()` + `finish_exact()` produces output byte-for-byte identical to `encode()`'s whole-image `EncodeOptions::idim` path — not just pixel-equivalent after decode, the same bar `test_streaming_encoder_matches_whole_file_encode_byte_for_byte` already set for the row-strip path), plus 9 error-path tests covering every documented failure mode (cross-calling `add_tile()`/`add_idim_tile()` in the wrong mode, wrong buffer size for the next tile's position, submitting past the last tile in the grid, incomplete `finish()`, invalid `scan_order`, zero tile dimensions, `bit_depth < 8` combined with iDIM, `use_filter_per_row` combined with iDIM, and `tiles_x * tiles_y` exceeding `MAX_TILE_COUNT`).

**Validation:** `cargo build --lib`, `cargo fmt --check`, `cargo clippy --all-targets -- -D warnings`, `cargo test --lib` (332 tests, unchanged — no lib-test-visible behavior changed, only new integration tests added), `cargo test --test streaming_encode` (30 tests, +12 from the pre-v1.10 baseline), `cargo test --doc` (2 doc-tests, unchanged) all pass with zero regressions. Manually verified end-to-end via `cargo run --example streaming_encode` in both row-strip (default) and iDIM (`8x8` row-major, `8x8:z` Z-order) modes against a generated PNG, decoded each output via `cafe-decode` to confirm correct round-trip.

### v1.11 - `Encoder<W>::add_even_odd_rows()` — even/odd interlacing (`Interlace = 2`) support for the streaming encoder

Revisits v1.9.1's investigation, which bundled Adam7 and even/odd together under "Interlace (Adam7/even-odd)" as a single permanent `Encoder<W>` limitation. That investigation's own reasoning already flagged the asymmetry: "Even/odd is structurally simpler and could in principle be supported per-tile alone, but doing so would introduce an asymmetry... inconsistent with how both interlace methods are treated at the same support level everywhere else in the codebase" — i.e. it was rejected for *consistency*, not because a real technical blocker existed. Re-examining that trade-off: even/odd's two passes (even rows, odd rows) are each independently, losslessly reconstructable from any subset of their own rows, with zero dependency on the other pass or on having the rest of the image available — unlike Adam7, whose 7 passes each scatter pixels non-contiguously across the *entire* image (a contiguous row-strip tile spans parts of every Adam7 pass, not one pass in isolation), which is a genuine, unavoidable blocker for streaming. The "treat both identically" consistency argument doesn't survive this asymmetry once decode-side precedent is considered too: `Decoder<R>::next_tile()` already treats interlace as a single non-streamable bucket for a *different*, mirror-image reason (an interlace pass isn't a spatial rectangle it could hand back as a `Tile`) — that reasoning applies equally to both Adam7 and even/odd on decode, but does not carry over to the encode side, where the direction of data flow is reversed and even/odd's per-row independence becomes exploitable in a way it isn't on decode.

- **`EncoderOptions::even_odd_interlace: bool`** (new field, `src/types.rs`; default `false`, non-breaking via the existing `..Default::default()` pattern): opts an `Encoder` into even/odd mode. A `bool` rather than an `Option<T>` since there is no accompanying configuration data (unlike `idim`'s tuple) — the two passes and their row-parity rule are fixed by the format, not caller-configurable.
- **`Encoder::new()`** (`src/cafe.rs`) validates upfront, before writing the signature or any chunk: rejects combination with `opts.idim.is_some()`, `use_filter_per_row`, and `use_byte_shuffle` (mirrors `encode()`'s own whole-image interlace-incompatibility checks — predictive-per-row filtering and byte-shuffle both require reordering/state that doesn't survive being split across two independently-flushed row-parity buffers); rejects any `sample_format`/`bit_depth`/`target_color_type` combination other than uint/8-bit/RGBA (the same restriction `encode()`'s whole-image interlace path already enforces, since interlace's pass-splitting logic assumes fixed 4-byte-per-pixel rows). On success, forces `filter_method = FILTER_METHOD_NONE` in the written `IHDR` (interlace is incompatible with every filter method, not just per-row/byte-shuffle — same rule as `encode()`), writes `INTERLACE_EVEN_ODD` into `IHDR`'s interlace byte, and initializes two new struct fields: `even_odd_interlace: bool` (mirrors the option, checked by `add_tile()`/`add_idim_tile()`'s guards) and `even_odd_pending: [Vec<u8>; EVEN_ODD_NUM_PASSES]` (one pending-row buffer per pass, indexed by `row_index % 2`).
- **`Encoder::add_even_odd_rows(&mut self, rgba_rows: &[u8]) -> Result<()>`** (new method): accepts a contiguous, top-to-bottom range of whole RGBA rows starting at the encoder's current absolute row cursor — deliberately *not* required to align to a pass boundary, an `EncoderOptions::tile_rows` multiple, or any fixed chunk size, so a caller can submit rows exactly as they become available (e.g. one row at a time, or the whole image in one call). Validates `rgba_rows.len()` is an exact multiple of one row's byte size and that the cumulative row count doesn't exceed `height`. Internally, walks the given rows one at a time, appending each to `even_odd_pending[row_index % 2]`; whenever a pass's pending buffer's row count reaches the `tile_rows` threshold, a new private `flush_even_odd_idat(pass)` helper drains it into one `IDAT` (prefixed with that pass's `pass_number` byte, then run through the same `apply_single_tile_filter`-adjacent pipeline as other tiles — trivial here since `filter_method` is always `FILTER_METHOD_NONE` for interlaced output — then `compress_with_fallback_dict` and written out), clearing the buffer back to empty. A private `flush_even_odd_full_tiles()` wraps the "check both passes, flush whichever crossed the threshold" loop so `add_even_odd_rows()`'s row-walking loop and `finish()`/`finish_exact()`'s final drain (below) share the same flush call, not two copies of similar logic.
- **Multiple `IDAT`s per pass**: because rows are bucketed and flushed independently per pass as they cross the threshold, a single even/odd pass may end up spread across **more than one** `IDAT` — a real difference from the whole-file `encode()` path, which always produces exactly one `IDAT` per pass regardless of `tile_rows` (confirmed empirically while writing `test_streaming_encoder_even_odd_matches_whole_file_encode_byte_for_byte`, see below). `handle_interlaced_idat()` (`src/cafe.rs`, decode side) was changed from overwriting a pass's row buffer on each matching `IDAT` to `extend_from_slice`-ing onto it, so multiple `IDAT`s sharing a `pass_number` now correctly concatenate instead of the later one silently clobbering the earlier one's rows. `compute_decompress_budget()`'s even/odd margin was widened from a flat `+EVEN_ODD_NUM_PASSES` (assumed exactly 2 `pass_number` bytes total, one per pass) to `+height` (one `pass_number` byte per row, a safe over-approximation of "one per potential `IDAT`," since a pass can never legitimately contain more `IDAT`s than it has rows) — the old flat margin would have under-counted the decompression budget for a file whose passes are split across many small `IDAT`s.
- **`add_tile()`/`add_idim_tile()`** both gained a guard returning `UnsupportedFeature` (pointing at `add_even_odd_rows()`) when called on an even/odd-mode `Encoder` — all three submission modes (row-strip, iDIM, even/odd) are now mutually exclusive, and cross-calling any pair is a caller error, consistent with the existing row-strip/iDIM mutual-exclusion pattern from v1.10.
- **New private `incomplete_message(&self) -> Option<String>` helper** replaces (and generalizes) v1.10's `is_complete() -> bool`: returns `None` when complete, or `Some(message)` naming the correct submission method for whichever of the three modes is active, when incomplete — `finish()`/`finish_exact()` both call it (returning `UnsupportedFeature` with the message when `Some`), and in even/odd mode, "complete" now also requires each pass's absolute row count submitted so far to equal `height`'s share of that parity (i.e. every declared row has been handed to `add_even_odd_rows()` at some point, though not necessarily yet flushed into an `IDAT` — see next point).
- **`finish()`/`finish_exact()`**: in even/odd mode, after the completeness check passes, a new private `flush_even_odd_remaining()` call drains whatever rows are still sitting in each pass's pending buffer (i.e. rows submitted but not yet reaching a full `tile_rows` threshold) into one final `IDAT` per **non-empty** pass, before writing `IEND` — a pass that happens to have already been fully flushed by threshold-crossing alone produces no extra empty `IDAT`. This mirrors `encode()`'s whole-file even/odd path's behavior of never emitting a zero-length pass `IDAT`, which is what let the byte-for-byte comparison test below succeed.
- **`EncoderOptions`'s doc comment** (`src/types.rs`) gained a paragraph revising the "Permanently out of scope" reasoning to split interlace into its two constituent methods: Adam7 (genuinely permanent, non-contiguous whole-image access) and even/odd (now supported, v1.11+); **`Encoder<W>`'s own doc comment** (`src/cafe.rs`) gained a "# Even/odd interlace (v1.11+)" section describing the `new()`/`add_even_odd_rows()` contract, multi-`IDAT`-per-pass behavior, and updated its "Permanent limitations" list to read "Adam7 interlacing" specifically instead of "interlace" generally.
- **New tests** (`tests/streaming_encode.rs`, +16): a new `encode_via_streaming_encoder_even_odd()` helper (analogous to the pre-existing `encode_via_streaming_encoder()`) parameterized by row-submission chunk size; `test_streaming_encoder_even_odd_finish_roundtrip_pixel_exact` / `test_streaming_encoder_even_odd_finish_exact_roundtrip_pixel_exact` (37×29 image, 8-row submission groups, via `finish()`/`finish_exact()` respectively); `test_streaming_encoder_even_odd_one_row_at_a_time_roundtrip` (21×17, exercises the finest-grained submission pattern); `test_streaming_encoder_even_odd_whole_image_single_call_roundtrip` (25×19, the coarsest pattern — entire image in one `add_even_odd_rows()` call); `test_streaming_encoder_even_odd_matches_whole_file_encode_byte_for_byte` (compares streaming output, submitted in 7-row groups with `tile_rows = height`, against `encode()`'s whole-file `Interlace = 2` output — confirmed byte-for-byte identical; the test's doc comment records the `tile_rows >= height` requirement discovered while writing it, since anything smaller causes the streaming side to legitimately split a pass across multiple `IDAT`s where the whole-file path never does); plus mode-guard tests (`test_streaming_encoder_add_tile_rejects_even_odd_mode`, `..._add_idim_tile_rejects_even_odd_mode`, `..._add_even_odd_rows_rejects_row_strip_mode`), buffer/height-validation tests (`..._rejects_wrong_size_buffer`, `..._rejects_height_overflow`), an incomplete-submission test (`..._finish_rejects_incomplete_submission`), and five `Encoder::new()` rejection tests covering every mutual-exclusion/format-restriction rule (`idim` combined with `even_odd_interlace`, `use_filter_per_row` combined with it, `use_byte_shuffle` combined with it, non-RGBA color type, non-default bit depth).

**Validation:** `cargo fmt` (3 formatting diffs auto-corrected, then `cargo fmt --check` clean), `cargo clippy --all-targets -- -D warnings` (zero warnings), `cargo test --lib` (332 tests, unchanged — no lib-test-visible behavior changed), `cargo test --test streaming_encode` (46 tests, +16 from v1.10's 30 — all passing), `cargo test` (full workspace — `integration_test.rs`, `palette_algorithm_test.rs`, `roundtrip_formats.rs`, and every other pre-existing integration suite re-verified with no regressions) all pass with zero regressions.
...
Closes the "Advanced 2D tiling"/`Encoder<W>` write-side item's remaining ambiguity from "Welcome Contributions" #3 and the `Encoder<W>` support for `auto_dictionary`/indexed/interlace" line previously sitting in this changelog's "Planned (Future)" section: those three sub-features were described as deferred ("out of scope for v1", "not yet implemented") without the codebase ever having actually investigated whether an incremental, buffer-free implementation was possible. This version performs that investigation and documents the conclusion — no code or runtime behavior changes.

- **`auto_dictionary`**: `train_zstd_dictionary` needs several already-*compressed* tile samples before it can train a useful dictionary, but applying that dictionary to the earliest tiles — where it would help compression the most, since it "warms up" ZSTD's context — would require not having already written them. Unlike `finish_exact()`'s single fixed-position `compression_method` byte patch, recompressing an already-written tile changes its length, shifting every subsequent byte in the file with no seek-and-patch remedy available for a variable-length field. The only fix is buffering the first N tiles before writing anything — a different contract for `add_tile()`, not an incremental extension of it. An explicit, caller-supplied `zstd_dictionary` (trained offline) remains fully supported and needs no such buffering.
- **Indexed palette (`COLOR_TYPE_INDEXED`)**: quantization needs a color histogram of the *entire* image, and `PLTE` must precede every `IDAT` per the spec's mandatory chunk order (section 9) — so the final palette must be complete before the first pixel is written. No incremental middle ground exists: either the whole image is buffered (making this indistinguishable from calling `encode_indexed()` directly, not a streaming API), or callers submit tiles twice (once to collect color statistics, once to encode against the now-known palette) — a fundamentally different two-pass API shape that would deserve its own type rather than a mode of today's single-pass `Encoder<W>`.
- **Interlace (Adam7/even-odd)**: Adam7's `extract_adam7_pass` (`src/interlace.rs`) reads the *entire* image buffer directly, because each of its 7 passes picks pixels scattered non-contiguously across the whole image — a contiguous row-strip tile spans parts of all 7 passes, not one pass in isolation, so generating any pass requires the whole image up front. Even/odd is structurally simpler (2 passes = even/odd rows, both already present within any contiguous row range) and could in principle be supported per-tile alone, but doing so would introduce an asymmetry (even/odd supported, Adam7 not) inconsistent with how both interlace methods are treated at the same support level everywhere else in the codebase (e.g. `Decoder<R>::next_tile()` rejects both equally, for the mirror-image reason — see v1.9's notes above). Both remain rejected together.
- **`EncoderOptions`'s doc comment** (`src/types.rs`) rewritten under a new "Permanently out of scope (investigated, decided against — not a 'v1' gap)" heading with the above per-item reasoning; **`Encoder<W>`'s own doc comment** (`src/cafe.rs`) updated to match and to point at `EncoderOptions`'s for the full analysis.
- Removed the "`Encoder<W>` support for `auto_dictionary`, indexed palette, and interlace" line from this document's/`CHANGELOG.md`'s "Planned (Future)"/roadmap sections — it will not become a future work item, since no viable buffer-free design was found. `iDIM` (2D tiling) support for `Encoder<W>`'s *write* side remains a separate, still-open item (unlike the other three, it doesn't require an unbounded-size histogram or non-contiguous whole-image access — only knowing the tile grid upfront, which is a smaller, more tractable gap) and stays listed under "Welcome Contributions" #3.

**Validation:** `cargo build --lib`, `cargo fmt --check`, `cargo clippy --all-targets -- -D warnings`, `cargo test --lib` (332 tests, unchanged from v1.9.0 — no test code touched), `cargo test --doc` (2 doc-tests, unchanged) all pass with zero regressions, confirming this is purely a documentation change.

### v1.9.2 - CI: ARM64 native test job

Closes the "Real hardware validation on physical ARM devices" item's remaining CI-automation gap flagged in v1.4.1's notes: that phase validated the NEON port by manually running the test suite once, locally, under QEMU emulation via Docker — it found and fixed a real bug, but was never wired into CI, so a future regression of the same kind could merge unnoticed until someone re-ran that manual QEMU session again.

- **New job**: `arm64-native-test` added to `.github/workflows/ci.yml`, running independently alongside the existing `test`, `clippy`, `fmt`, `fuzz`, `aarch64-cross-compile`, `compression-regression`, `security-audit`, and `doc` jobs on every push/PR.
- **`runs-on: ubuntu-24.04-arm`**: a GitHub-hosted Actions runner backed by real Arm-based server CPUs (Azure Cobalt 100), not x86_64 with QEMU emulation layered on top — the same category of validation gap v1.4.1 closed manually, now automated. This is meaningfully different from `aarch64-cross-compile` (which only ever runs `cargo check`/`clippy` — the NEON intrinsics are type-checked but never actually executed) and from v1.4.1 itself (which did execute real aarch64 code, but as a one-off local Docker session, not on every push/PR).
- **Commands**: `cargo test --lib --release` (332 tests — architecture-specific AVX2-vs-scalar comparison tests are skipped, mirroring the 268/273 count difference already observed and documented in v1.4.1) followed by `cargo test --release` (full integration suite, matching the existing x86_64 `test` job's own two-step pattern).
- **Toolchain/setup**: `dtolnay/rust-toolchain@stable` (no cross-compiler needed — the runner's own `gcc` is aarch64-native, unlike `aarch64-cross-compile`'s x86_64 host needing `gcc-aarch64-linux-gnu`), `Swatinem/rust-cache@v2` keyed on `arm64-native` to avoid clashing with the `aarch64-unknown-linux-gnu` cache key already used by `aarch64-cross-compile`.
- **`timeout-minutes: 30`**: generous enough for a cold dependency build plus the full test suite on a (likely) less cache-warm runner pool than the long-established `ubuntu-latest` pool, without being so long that a genuinely hung job wastes CI minutes indefinitely.
- **Scope note, restated from v1.4.1/v1.4.2**: this job closes the "does the code actually execute correctly on real ARM64 hardware, automatically, on every change" gap. It does **not** close the separate, still-open "Welcome Contributions" item of validating on specific *end-user* device classes (Raspberry Pi's Cortex-A72/A76, mobile SoCs, Apple Silicon's M-series) — cloud server-grade Arm CPUs may differ from those in cache sizes, memory bandwidth, or alignment-sensitive edge cases QEMU or a different physical device might expose. The roadmap/changelog "Planned (Future)" wording was adjusted to make this distinction explicit rather than conflate the two.

**Validation:** workflow YAML syntax checked with `rhysd/actionlint` (via Docker, `docker run --rm -v <repo>:/repo rhysd/actionlint`) against the full `ci.yml` — zero errors/warnings. The job's actual execution on `ubuntu-24.04-arm` was not exercised end-to-end locally (that requires pushing to trigger a real Actions run using that runner label, which isn't available to invoke synchronously from this environment); confidence instead comes from `actionlint`'s schema/syntax validation plus close mirroring of the already-proven-working `aarch64-cross-compile`/`test`/`fuzz` jobs' toolchain setup and command patterns. `cargo build --lib`, `cargo fmt --check`, `cargo clippy --all-targets -- -D warnings`, `cargo test --lib` (332 tests, unchanged — no Rust source touched by this change) all still pass locally on x86_64, confirming zero regressions from the version bump.

### v1.9 - `Decoder<R>::next_tile()` support for 2D tiling (`iDIM`)

Closes the "Advanced 2D tiling" item from "Welcome Contributions" #3 for the decode side: `iDIM` (2D tiling) already had full whole-image round-trip support (`encode`/`decode_bytes` with row-major and Z-order scan) since v1.1, but `Decoder<R>::next_tile()` (the streaming decode API added in v1.5) unconditionally rejected any file containing an `iDIM` chunk with `CafeError::UnsupportedFeature`, forcing a fallback to the whole-image API for exactly the kind of large/tiled image streaming decode is most useful for.

- **`decode_idim_tile_raw`** (new private helper, `src/cafe.rs`): factored out of `handle_idat_tile_idim`'s existing per-tile logic — validates `COLOR_TYPE_INDEXED`/`bit_depth < 8` are rejected, looks up the tile's `(tx, ty)` grid position from `state.idim_tile_order` (cached from the `iDIM` chunk, in whichever `scan_order` the file declares), computes `(tile_width, tile_height)` via `iDim::tile_dimensions` (narrower/shorter at right/bottom edges for non-tile-size-multiple images), and reverses byte-shuffle/predictive filtering. Both `handle_idat_tile_idim` (whole-image path, now just calls this and copies the result into `state.pixel_rows`) and the new streaming functions below call it — tile geometry and filter-reversal logic cannot diverge between the two paths.
- **`decode_idat_as_tile_idim` / `decode_idat_chunk_as_tile_idim`** (new private functions): the `iDIM` analogues of the existing `decode_idat_as_tile_row_strip` / `decode_idat_chunk_as_tile_row_strip`, converting one tile's raw bytes to RGBA (via the same shared `convert_raw_to_rgba`) and returning a `Tile` positioned at `x = tx * tile_width, y = ty * tile_height` — critically, without ever touching `state.pixel_rows` (no whole-image buffer is allocated for the streaming path).
- **`Decoder::next_tile()`**: now checks `self.state.idim.is_some()` and dispatches to `decode_idat_chunk_as_tile_idim` instead of the row-strip function when true. `DecodeInfo::supports_streaming_tiles` correspondingly changes from `interlace == NONE && idim.is_none()` to `interlace == NONE && (idim.is_none() || (color_type != INDEXED && bit_depth >= 8))` — the same two restrictions the whole-image `iDIM` path already enforced.
- **Interlace (Adam7/even-odd) remains permanently unsupported by `next_tile()`** — a deliberate, documented design limitation, not a "not yet implemented" placeholder: an interlace pass is not a spatial rectangle (each pass touches every row/column of the full image at a stride), so it cannot become a standalone RGBA `Tile` without every other pass also being decoded first, which defeats the point of a streaming, constant-memory API. `DecodeInfo::supports_streaming_tiles` stays `false` for interlaced files and `next_tile()` still returns `Err(UnsupportedFeature)` for them; callers should keep falling back to `decode_bytes`/`decode` for interlaced files specifically, as documented in `Decoder`'s doc comment.
- **New tests** (`src/cafe.rs`): `test_decoder_next_tile_loop_matches_whole_image_decode_idim` (non-power-of-two 33×23 image, 8×8 tiles, row-major — exercises partial edge tiles), `test_decoder_next_tile_loop_matches_whole_image_decode_idim_zorder` (same, `scan_order=1`), `test_encode_indexed_rejects_idim` (pins the pre-existing encoder-side rejection that makes an indexed+iDIM combination unreachable via any CAFE encoder), `test_decode_adversarial_idim_with_indexed_color_type` (hand-crafted file bypassing the encoder confirms the decoder itself also rejects the combination, not just the encoder), and `test_decoder_next_tile_rejects_interlaced_adam7` (replaces the now-obsolete `test_decoder_next_tile_rejects_idim_2d_tiling`, isolating the still-unsupported interlace case). New `assert_idim_tiles_reassemble_to` helper checks both pixel-exact reassembly and full non-overlapping pixel coverage across all yielded tiles.

**Validation:** `cargo build --lib`, `cargo fmt --check`, `cargo clippy --all-targets -- -D warnings`, `cargo test --lib` (332 tests, +4 from v1.8.0's 328), `cargo test` (full workspace, including `roundtrip_formats.rs`'s pre-existing `iDIM` whole-image round-trip tests and doc-tests) all pass with zero regressions.

### v1.8 - Inverse tone-mapping on encode (SDR→HDR synthesis)

Closes the "Tone-mapping on encode (SDR→HDR)" item from "Welcome Contributions" #7: decode already supported forward tone-mapping (EOTF PQ/HLG/sRGB → linear → compress via Reinhard/Filmic, v1.1) but encode had no way to go the other direction — RGBA float samples were always produced via the naive `v/255` linear scaling, never a real inverse tone-map.

- **`ToneMapOperator::apply_inverse(&self, y: f32) -> Result<f32>`** (`src/tonemap.rs`): the mathematical inverse of `apply()` for `Reinhard` (`y/(1-y)`, clamped so `y` must be in `[0, 1)`); `Filmic` returns `CafeError::UnsupportedFeature` — the ACES-approximation curve used by `apply()` has no closed-form inverse (it's a rational function with no analytically invertible form), so silently guessing would misrepresent HDR values rather than fail loudly.
- **`inverse_tonemap_sdr` (private) / `apply_inverse_tone_mapping_to_image` (public)**: mirrors the forward decode pipeline (`tonemap_hdr`/`apply_tone_mapping_to_image`) in reverse — `srgb_eotf` (undo sRGB companding) → `operator.apply_inverse` (decompress) → `convert_primaries` (source→target gamut via XYZ, same matrices as decode) → `× max_luminance` (denormalize to nits). Takes 8-bit RGBA input, source color primaries, `cHDR` metadata, and an operator; returns RGBA float32 (big-endian, 16 bytes/pixel) — alpha passes through unconverted (already `[0,1]`-normalized by simple division).
- **Opt-in `EncodeOptions::inverse_tonemap: Option<ToneMapOperator>` field** (default `None`, so existing callers/behavior are unaffected — this is non-breaking despite adding a struct field, since `EncodeOptions` is always constructed via `..EncodeOptions::default()` in every call site including the CLI). Deliberately **not** automatic even when `sample_format=float` + `chdr_metadata` are both set, to avoid silently changing already-existing HDR-encode behavior for callers who set those two fields today without expecting SDR→HDR synthesis.
- **Validation in `encode()`** (upfront, before any pixel conversion work): rejects with `CafeError::UnsupportedFeature` unless `sample_format == SAMPLE_FORMAT_FLOAT` (not half — mirrors `convert_raw_to_rgba`'s own restriction of tone-mapping to float on the decode side, so an encode+decode round-trip is coherent), `target_color_type == COLOR_TYPE_RGBA`, `chdr_metadata` is `Some`, and `chdr.transfer_function == 0` (linear only — no OETF re-encoding to PQ/HLG/sRGB gamma is implemented on the encode side, only the linear-light case matching decode's own linear branch).
- **CLI**: `--inverse-tonemap <reinhard>` on `cafe-encode` (parsed via `ToneMapOperator::from_str`, `filmic` rejected at parse time with a clear message rather than deferred to `encode()`'s own error).
- **Pre-existing CLI bug found and fixed while wiring this in**: `cafe-encode`'s auto-detection of "few unique colors → route to `encode_indexed()`" ran *before* `sample_format`/`chdr_metadata`/`inverse_tonemap` were consulted, silently discarding those flags whenever an input image happened to have few colors (since `encode_indexed()` has no HDR code path at all). Fixed by moving the HDR-flags check earlier: `--indexed` explicitly combined with any HDR flag is now a hard parse error (the combination is fundamentally unsupported, not just currently unwired), and auto-detection skips the indexed path (falls through to `encode()`) whenever HDR flags are present without an explicit `--indexed`.
- **Known limitation (mathematical, not a bug)**: Reinhard's forward `apply(x) = x/(1+x)` on the linear-transfer branch of `tonemap_hdr` only ever produces outputs in `[0, 0.5]`, corresponding to sRGB-encoded 8-bit inputs up to ~187/255 — brighter source pixels have no exact round-trip through `apply_inverse` followed by decode's `apply`, since the forward compression curve itself never reaches those output values from that domain. This is inherent to the operator, documented in `apply_inverse`'s doc comment and in the round-trip test's comment, not something `apply_inverse` could "fix".
- **New tests**: 10 unit tests in `src/tonemap.rs` (`apply_inverse` correctness/round-trip/domain-clamping for Reinhard, `Filmic` rejection, `inverse_tonemap_sdr`/`apply_inverse_tone_mapping_to_image` end-to-end including non-linear-transfer-function rejection and buffer-size validation) and 6 integration tests in `tests/integration_test.rs` (real `encode()`+`decode_with_opts()` round-trip with `max_diff<=3`/channel via a new `make_dim_test_image()` helper kept within Reinhard's round-trippable domain, plus one rejection test per validation rule: uint sample format, missing `chdr_metadata`, non-linear `transfer_function`, `Filmic` operator, non-RGBA color type).

**Validation:** `cargo fmt --check`, `cargo clippy --all-targets -- -D warnings`, `cargo test --lib` (328 tests, +11 from v1.7's 317), `cargo test --test integration_test` (all passing, +6 new) all pass with zero regressions; `benches/encode_decode.rs` updated to include the new `inverse_tonemap: None` field (compile fix, `E0063`, no behavioral change). Manually verified end-to-end via release binaries: `cafe-encode --sample-format 1 --chdr-transfer 0 --chdr-max-lum 1000 --inverse-tonemap reinhard` followed by `cafe-decode --tonemap-operator reinhard` round-trips correctly through the RGBA-float path (not misrouted to `encode_indexed()`); `--indexed` combined with any HDR flag confirmed to fail fast with a clear error; the pre-existing no-HDR-flags auto-indexed-detection behavior confirmed unchanged.
