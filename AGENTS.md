...
3. **Advanced 2D tiling** — iDIM with per-tile IDAT already implemented (row-major and Z-order), including streaming decode via `Decoder<R>::next_tile()` (v1.9); evolve with preview/progressive streaming, and `Encoder<W>` write-side support (currently row-strip only, no iDIM)
