...
fn test_auto_dictionary_roundtrip() {
    ...
    {
        let mut opts = cafe::EncodeOptions::default();
        opts.use_filter = true;
        opts.level = 9;
        opts.target_color_type = 6; // RGBA
        opts.target_bit_depth = Some(8);
        opts.auto_dictionary = true; // Enable auto-dictionary
    ...
    let cafe_no_auto_dict = "target/test_no_auto_dict.cafe";
    {
        ...
        opts.target_color_type = 6; // RGBA
        opts.target_bit_depth = Some(8);
        opts.auto_dictionary = false; // Disabled
        
        let result = cafe::encode(png_path_str, cafe_no_auto_dict, &opts);
        assert!(result.is_ok(), "encode without auto_dictionary should succeed");
    }
    ...
    assert!(result_no_auto.is_ok(), "decode without auto_dict should succeed");
    
    // Verify that both decoded files exist
    assert!(Path::new(decoded_auto_dict).exists(), "decoded auto_dict file should exist");
    assert!(Path::new(decoded_no_auto_dict).exists(), "decoded no_auto_dict file should exist");
