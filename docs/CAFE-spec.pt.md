...
Simétrico ao decode em streaming: um encoder pode escrever o `IHDR` e cada `IDAT` de tile em faixas de linhas incrementalmente à medida que os tiles ficam disponíveis, em vez de exigir a imagem inteira em memória antes de produzir qualquer saída. O `Encoder<W: Write>` da implementação de referência suporta isso apenas para color types diretos (`cinza`/`RGB`/`cinza+alfa`/`RGBA` — não indexado, já que a quantização de paleta precisa ver todo pixel antes que um único índice possa ser emitido) e apenas tiling em faixas de linhas (não tiling 2D via `iDIM`, que precisa da grade de tiles completa antecipadamente, nem entrelaçamento Adam7/par-ímpar, que precisa dos pixels da imagem inteira para intercalar).
...
| 11 | Compression method | bitmask: bit0=ZSTD, demais bits reservados para algoritmos futuros |
