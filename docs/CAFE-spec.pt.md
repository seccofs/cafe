...
- **Uso real na compressão (funcional, não decorativo)**: quando presente, **todo** chunk `IDAT` com `Flag = 0x01` (ZSTD) do arquivo foi comprimido usando esse dicionário — não é permitido misturar `IDAT` com e sem dicionário no mesmo arquivo. O decoder deve fornecer o mesmo dicionário ao descompressor ao processar qualquer `IDAT`.
...
- Tamanho de tile é um trade-off: tiles menores = streaming mais granular, porém mais overhead de cabeçalho/CRC por chunk **e** menor eficiência do filtro preditivo (seção 4.3.1), já que cada tile reinicia a predição na primeira linha.
