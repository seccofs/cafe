...
**Múltiplos `IDAT`s por passe (apenas par/ímpar, v1.11+):** o caminho de encode de arquivo completo (`encode()`) sempre emite exatamente um `IDAT` por passe, tanto para `Interlace = 1` quanto para `Interlace = 2`, como descrito acima. O encoder em streaming (`Encoder<W>`, seção 6.1) pode adicionalmente dividir as linhas de um único passe par/ímpar em **múltiplos** `IDAT`s, cada um ainda prefixado com o `pass_number` daquele passe, à medida que as linhas chegam incrementalmente e a contagem de linhas em buffer de um passe ultrapassa o limiar de linhas por tile configurado — um decoder deve, portanto, **concatenar** (na ordem de chegada) os payloads de todo `IDAT` que compartilhe o mesmo `pass_number` par/ímpar, em vez de assumir exatamente um `IDAT` por passe. Isso é um esclarecimento de comportamento do lado do decoder, não uma mudança no formato binário: um arquivo anterior à v1.11 (sempre um `IDAT` por passe par/ímpar) concatena sobre um buffer inicialmente vazio, o que equivale ao que aceitar apenas um `IDAT` por passe já fazia. O Adam7 não é afetado — o decoder de referência ainda espera (e o encoder de referência ainda só produz) exatamente um `IDAT` por passe Adam7, já que o streaming de Adam7 está fora do escopo do `Encoder<W>` (a nota de design da seção 6.1 explica o porquê).
...
- **Entrelaçamento par/ímpar (`Interlace = 2`, desde v1.11)**: o chamador opta por isso via `EncoderOptions::even_odd_interlace = true` (restrito a RGBA uint de 8 bits, mesma restrição que o interlace já tem no caminho de arquivo completo — ver seção 4.1.2) e submete faixas contíguas de linhas de altura e alinhamento arbitrários via `add_even_odd_rows()` — diferente do Adam7, os dois passes do par/ímpar (linhas pares, linhas ímpares) são cada um independente e losslessly reconstruível a partir de qualquer subconjunto de linhas sem precisar do resto da imagem, então um encoder pode distribuir cada linha recebida no buffer pendente do seu próprio passe e liberar o buffer de um passe em um `IDAT` (prefixado com o `pass_number` daquele passe, conforme a seção 5) sempre que ultrapassar o limiar de linhas por tile configurado — ver a nota da seção 5 acima sobre a possibilidade resultante de múltiplos `IDAT`s por passe, e a linha da seção 6 sobre por que isso não se aplica ao Adam7.
