...
**CAFE Format Version:** 1.0 (frozen 2026-09-04, independent of the implementation's own version — see `docs/CAFE-spec.md` section 13 and `AGENTS.md`'s compatibility table)
...
impl<W: Write> Encoder<W> {
    ...
    pub fn add_idim_tile(&mut self, rgba_tile: &[u8]) -> Result<()> // v1.10+, one 2D tile per call
...
**Call order (row-strip mode, default)**: `new()` once (writes signature +
`IHDR` + pre-IDAT ancillary chunks immediately), then `add_tile()` any
number of times with row-strip buffers (`width * tile_height * 4` bytes
RGBA — `tile_height` is inferred from the buffer, not required to be
constant across calls), then `finish()`/`finish_exact()` once all `height`
rows have been submitted.

**Call order (2D tiling / `iDIM` mode, v1.10+)**: set
`EncoderOptions::idim: Some((tile_width, tile_height, scan_order))` before
calling `new()` — this writes the `iDIM` chunk immediately after `IHDR` and
precomputes the full tile grid order (row-major, `scan_order=0`, or Z-order,
`scan_order=1`). Then call `add_idim_tile()` exactly once per tile, in that
precomputed order, with a buffer sized to that tile's exact
`tile_w * tile_h * 4` bytes (narrower/shorter at right/bottom edges when
`width`/`height` aren't exact multiples of the declared tile size). `add_tile()`
and `add_idim_tile()` are mutually exclusive per `Encoder` instance — calling
the wrong one for the current mode returns `UnsupportedFeature`. Finish via
`finish()`/`finish_exact()` as usual once every tile in the grid has been
submitted.

**Limitations (permanent, investigated — not "v1 gaps")**: no indexed
palette (`COLOR_TYPE_INDEXED` — quantization needs the whole image's color
histogram upfront, and `PLTE` must precede every `IDAT`; `encode_indexed()`
remains the only path for that), no interlace (Adam7/even-odd — Adam7's
passes are non-contiguous across the whole image, not per-tile), no
`auto_dictionary` (recompressing already-written early tiles once a
dictionary is trained would shift every subsequent byte, with no
seek-and-patch remedy for a variable-length field). 2D tiling (`iDIM`) is
**not** in this list — see `EncoderOptions`'s doc comment for why it turned
out not to need whole-image buffering, unlike the three above. An explicit,
caller-supplied `zstd_dictionary` remains supported in both submission modes.
...
| **v1.9.3** | **Documentation-only: `compression_method` spec semantics clarified** — new normative note in spec section 4.1 states `compression_method`'s `bit0` is a required-lower-bound capability declaration, never a per-chunk record (that's `Flag`'s job); new "Decoder conformance note" documents that the reference decoder doesn't cross-validate the two, flagging the interoperability implication for other implementations | ✅ |
| **v1.10** | **`Encoder<W>::add_idim_tile()` — 2D tiling (`iDIM`) support for the streaming encoder**: symmetric encode-side counterpart to v1.9's streaming-decode `iDIM` support; corrects v1.9.1's classification of `iDIM` as a permanent `Encoder<W>` limitation (only geometry — `tile_width`/`tile_height`/`scan_order` — is needed upfront, not pixel data); new `EncoderOptions::idim: Option<(u16, u16, u8)>` field (default `None`, non-breaking), row-major and Z-order both supported, `add_tile()`/`add_idim_tile()` mutually exclusive | ✅ |
| **v1.11** | **`Encoder<W>::add_even_odd_rows()` — even/odd interlacing (`Interlace = 2`) support for the streaming encoder**: revisits v1.9.1's bundling of Adam7 and even/odd as a single permanent `Encoder<W>` limitation — even/odd's two passes are each independently, losslessly reconstructable from any row subset (unlike Adam7, which genuinely can't be streamed); new `EncoderOptions::even_odd_interlace: bool` field (default `false`, non-breaking, mutually exclusive with `idim`/`use_filter_per_row`/`use_byte_shuffle`), restricted to 8-bit uint RGBA; `add_even_odd_rows()` accepts arbitrary-height contiguous row ranges and may split a single pass across multiple `IDAT`s; Adam7 remains permanently unsupported by `Encoder<W>` | ✅ |
...
3. **Advanced 2D tiling** — iDIM with per-tile IDAT already implemented, including streaming decode via `Decoder<R>::next_tile()` (v1.9) and streaming encode via `Encoder<W>::add_idim_tile()` (v1.10); evolve with preview/progressive streaming, and richer per-tile progress/callback hooks
...
**Last updated:** September 4, 2026 (v1.12: CAFE Format Version formalized and decoupled from the `cafe-rs` implementation version — new `FORMAT_VERSION_MAJOR`/`FORMAT_VERSION_MINOR` constants, `-V`/`--version` CLI flag on both tools, new `docs/CAFE-spec.md`/`.pt.md` section 13 declaring CAFE Format 1.0 frozen (retroactively covering v1.0–v1.11's on-disk additions), new `AGENTS.md` implementation-to-format compatibility table; documentation and tooling only, no on-disk/behavioral change; v1.11: `Encoder<W>::add_even_odd_rows()` — even/odd interlacing (`Interlace = 2`) support for the streaming encoder, revisiting v1.9.1's bundling of Adam7/even-odd as a single limitation since even/odd's passes are independently reconstructable per-row; new opt-in `EncoderOptions::even_odd_interlace` field, mutually exclusive with `idim`/`use_filter_per_row`/`use_byte_shuffle`; Adam7 remains permanently unsupported; v1.10: `Encoder<W>::add_idim_tile()` — 2D tiling (`iDIM`) support for the streaming encoder, symmetric counterpart to v1.9's streaming decode; new opt-in `EncoderOptions::idim` field, row-major and Z-order both supported; v1.9.3: `compression_method` spec semantics clarified — normative "required lower bound" definition and decoder conformance note added to section 4.1, documentation only, no code change; v1.9.2: CI gains an `arm64-native-test` job running the full test suite natively on `ubuntu-24.04-arm` (real ARM64 silicon, not emulated); v1.9.1: `Encoder<W>`'s `auto_dictionary`/indexed/interlace limitations reclassified from "v1 gap" to permanent, investigated design limitation — documentation only, no code change; v1.9: `Decoder<R>::next_tile()` streaming support for 2D tiling — `iDIM` files now stream real `(x, y, width, height)` tiles instead of erroring, interlace remains a permanent documented limitation; v1.8: inverse tone-mapping on encode — `EncodeOptions::inverse_tonemap`, `--inverse-tonemap reinhard`, SDR→HDR synthesis; v1.7: `PaletteAlgorithm::KMeans` — deterministic k-means palette quantization, `--palette-algorithm kmeans`; v1.6.3: nightly fuzz CI workflow — `.github/workflows/fuzz.yml`; v1.6.2: real `compression_stats` tracking + `cafe-decode` gains `--show-stats`/`--save-exif`/`--save-icc-profile`/`--save-xmp`/`--save-zstd-dict` flags; v1.6.1: `cafe-encode` gains `--icc-profile-file`/`--xmp-file` CLI flags; v1.6: streaming encoder — `Encoder<W: Write>` / `Encoder<W: Write + Seek>`, symmetric counterpart to the v1.5 `Decoder<R: Read>`) | **Project version:** v1.12.0
